...
bool NET_PRES_EncProviderStreamClientInit0(NET_PRES_TransportObject * transObject) {
    ...
    net_pres_wolfSSLInfoStreamClient0.wolfssl_context = wolfSSL_CTX_new(wolfSSLv23_client_method());
    if (net_pres_wolfSSLInfoStreamClient0.wolfssl_context == 0) {
        return false;
    }
    wolfSSL_SetIORecv(net_pres_wolfSSLInfoStreamClient0.wolfssl_context, (CallbackIORecv) & NET_PRES_EncGlue_StreamClientReceiveCb0);
    wolfSSL_SetIOSend(net_pres_wolfSSLInfoStreamClient0.wolfssl_context, (CallbackIOSend) & NET_PRES_EncGlue_StreamClientSendCb0);
    if (wolfSSL_CTX_load_verify_buffer(net_pres_wolfSSLInfoStreamClient0.wolfssl_context, caCertsPtr, caCertsLen, SSL_FILETYPE_ASN1) != SSL_SUCCESS) {
        // Couldn't load the certificates
        //NPEG_DEBUG_PRINT("NETPRE: Something went wrong loading the certificates\r\n");
        wolfSSL_CTX_free(net_pres_wolfSSLInfoStreamClient0.wolfssl_context);
        return false;
    }
    // Turn off verification, because SNTP is usually blocked by a firewall
    wolfSSL_CTX_set_verify(net_pres_wolfSSLInfoStreamClient0.wolfssl_context, SSL_VERIFY_NONE, 0);
    net_pres_wolfSSLInfoStreamClient0.isInited = true;
    return true;
}

bool NET_PRES_EncProviderStreamClientDeinit0() {
    NPEG_DEBUG_PRINT(" WolfSSL DeInit\r\n");
    wolfSSL_CTX_free(net_pres_wolfSSLInfoStreamClient0.wolfssl_context);
...
bool NET_PRES_EncProviderStreamClientOpen0(uintptr_t transHandle, void * providerData) {
    NPEG_DEBUG_PRINT(" WolfSSL Open\r\n");
    WOLFSSL* ssl = wolfSSL_new(net_pres_wolfSSLInfoStreamClient0.wolfssl_context);
    if (ssl == NULL) {
        return false;
    }
...
NET_PRES_EncSessionStatus NET_PRES_EncProviderClientConnect0(void * providerData) {
    ...
    switch (result) {
        case SSL_SUCCESS:
            NPEG_DEBUG_PRINT(" WolfSSL Connect\r\n");
...
NET_PRES_EncSessionStatus NET_PRES_EncProviderConnectionClose0(void * providerData) {
    WOLFSSL* ssl;
    NPEG_DEBUG_PRINT(" WolfSSL Close\r\n");
    memcpy(&ssl, providerData, sizeof (WOLFSSL*));
    wolfSSL_free(ssl);
...
int32_t NET_PRES_EncProviderWrite0(void * providerData, const uint8_t * buffer, uint16_t size) {
    WOLFSSL* ssl;
    //NPEG_DEBUG_PRINT(" WolfSSL Write %d\r\n", size);
    memcpy(&ssl, providerData, sizeof (WOLFSSL*));
...
uint16_t NET_PRES_EncProviderWriteReady0(void * providerData, uint16_t reqSize, uint16_t minSize) {
    ...
    if (ret == 0) { // success
        //NPEG_DEBUG_PRINT(" WolfSSL Write Ready %d\r\n", reqSize);
        return reqSize;
    }
    if (minSize != 0) {
...
int32_t NET_PRES_EncProviderRead0(void * providerData, uint8_t * buffer, uint16_t size) {
    ...
    //NPEG_DEBUG_PRINT(" WolfSSL Read n = %d\r\n", ret);
    if (ret < 0) {
        return 0;
    }
    return ret;

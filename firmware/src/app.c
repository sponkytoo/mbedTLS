...
void APP_Tasks ( void )
{
    ...
    switch ( appData.state )
    {
        ...
        case APP_TCPIP_SEND_REQUEST:
        {
            ...
            SYS_CONSOLE_PRINT("Send Clear Text GET\r\n");
        ...
        {                        
            sprintf(networkBuffer, "GET / HTTP/1.0\r\n\r\n");
//            
            sprintf(networkBuffer, "GET /%s HTTP/1.1\r\n"
                    "Host: %s\r\n"
                    "Connection: close\r\n\r\n", appData.path, appData.host);
        ...
        {
            int len;
            if ((len = NET_PRES_SocketReadIsReady(appData.socket)) == 0)
            {
                if (NET_PRES_SocketWasReset(appData.socket))
                {
                    SYS_CONSOLE_PRINT("Socket was Reset\r\n");
                    appData.state = APP_TCPIP_CLOSE_CONNECTION;
                }
                break;
            }
            ...
            appData.lastRxDataPacket = SYS_TMR_SystemCountGet();                          
            revc_buffer = 0;
            revc_buffer = malloc(len);
            if(revc_buffer==0)
            {
                SYS_CONSOLE_PRINT("MALLOC PROBLEM\r\n");
                appData.state = APP_TCPIP_CLOSE_CONNECTION;
                break;
            }
            uint16_t res = NET_PRES_SocketRead(appData.socket, revc_buffer, len);
            appData.clearBytesReceived += res;
            appData.rawBytesReceived += res;
            if(revc_buffer)free(revc_buffer);
            revc_buffer = 0;
            break;
        }
        ...
        {
            SYS_CONSOLE_PRINT("Close Connection\r\n");
            vTaskDelay(100 / portTICK_PERIOD_MS);            
            NET_PRES_SocketClose(appData.socket);
            SYS_CONSOLE_PRINT("Socket  closed\r\n");
            vTaskDelay(100 / portTICK_PERIOD_MS);
            if(revc_buffer){
